				...
				<div className={`${styles.flexCenter} flex-wrap`}>
					{bookmark.length ? (
						bookmark.map((item) => (
							<div
								key={item.id}
								className={`m-3 xs:w-[44%] sm:w-[250px] w-full h-[140px] flex flex-col bg-center bg-cover bg-no-repeat text-secondary rounded-[15px] cursor-pointer py-4 px-2 relative`}
								style={{
									background: `linear-gradient(180deg, ${item.color} 0%, rgba(0, 0, 0, 0.85) 100%) `,
								}}
							>
								<div
									className={` ${styles.flexBtw} w-full font-firaSans text-text_color text-[11px]`}
								>
								...
								<button
									className="absolute bottom-[10px] right-[5px] cursor-pointer text-[20px]"
						...
						>
							<h2 className="font-semibold">
								...
								some templates and check back!
							</h2>
							<div className="mb-5 mt-10">
								<img
									src={image}
									alt="bookmark a template"
								/>
							</div>
						</div>
					)}
