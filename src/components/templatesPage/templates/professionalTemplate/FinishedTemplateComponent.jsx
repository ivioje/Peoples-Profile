...
const FinishedTemplateComponent = ({ hideImage }) => {
    const { selectedColor, summaryContent, personalDetails, workExperience, skillContent, education, inputValues,
        selectedFile } = useContext(TemplateContext);
    return (
        <section id='template' className={`${styles.flexAround} sm:flex-row flex-col p-3 w-full sm:h-[1200px]  h-full aspect-square font-poppins text-text_color`}>
            <div className={`p-5 sm:w-[30%] text-[14px] w-full sm:h-full ${styles.flexStart} flex-col text-dimWhite text-opacity-90`}
                style={{ 'backgroundColor': selectedColor }}>
                <div className={`mt-6 w-[200px] h-[200px] ${hideImage ? 'hidden' : 'block'}`}>
                    {selectedFile ? (
                        <img
            ...
            <div className={`sm:w-[70%] w-full sm:h-full flex justify-start items-start flex-col tmp_content px-3 bg-white sm:pb-0 pb-10 `}>
                        ...
                        <div key={index}>
                            <div className={`${styles.flexBtw} mt-6 py-1`}>
                                ...
                                <p className='text-[11px] text-gray-500'>
                                    {`${new Date(data.startDate).toLocaleString('en-US', { year: 'numeric', month: 'short' })} - ${new Date(data.endDate).toLocaleString('en-US', { year: 'numeric', month: 'short' })}`}
                                </p>                                    </div>
                            <h3 className='text-[15px] capitalize'>{data.jobTitle} - <span>{`${data.city}, ${data.country}`}</span></h3>
                ...
                <div className={`my-4 border border-b-0 border-l-0 border-r-0 border-t-2 w-full h-full`}>
                    ...
                    {education.map((data, index) => (
                        <div key={index}>
                            ...
                            <h3 className='text-[15px] capitalize'>{data.course} <span>{`- ${data.honours} ${data.qualification}`}</span></h3>
                        </div>
                    ))}
                </div>
            </div>
